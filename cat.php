...
if($cid != "")
{
	...
	if($CATID != "" && is_numeric($CATID))
	{
		...
		$query1 = "SELECT count(*) as total from posts where active='1' AND category='".mysql_real_escape_string($CATID)."' $addsql order by PID desc limit $config[maximum_results]";
		$query2 = "SELECT A.*, B.seo, C.username from posts A, categories B, members C where A.active='1' AND A.category='".mysql_real_escape_string($CATID)."' $addsql2 AND A.category=B.CATID AND A.USERID=C.USERID order by A.feat desc, $dby limit $pagingstart, $config[items_per_page]";
