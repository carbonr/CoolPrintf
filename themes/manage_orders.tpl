                  ...
                  {/if}
                    ...
                    <div class="wrapper">
                      <div class="page-title">
                        <h2>{$lang154}<span>{$lang199}</span></h2>
                      </div>
                      <div class="metadata">
                        <ul class="filters">                            
                            <li>{if $s eq "active"}<span>{$lang200} <u>{$o|@count}</u></span>{else}<a href="{$baseurl}/manage_orders?s=active&b={$b}&a={$a}"><span>{$lang200} </span></a>{/if}</li>
                          <li>{if $s eq "delivered"}<span>{$lang201} <u>{$o|@count}</u></span>{else}<a href="{$baseurl}/manage_orders?s=delivered&b={$b}&a={$a}"><span>{$lang201} </span></a>{/if}</li>
                          <li>{if $s eq "completed"}<span>{$lang202} <u>{$o|@count}</u></span>{else}<a href="{$baseurl}/manage_orders?s=completed&b={$b}&a={$a}"><span>{$lang202} </span></a>{/if}</li>
                          <li class="last">{if $s eq "cancelled"}<span>{$lang203} <u>{$o|@count}</u></span>{else}<a href="{$baseurl}/manage_orders?s=cancelled&b={$b}&a={$a}"><span>{$lang203} </span></a>{/if}</li>
                        </ul>
                      </div>
                      {if $o|@count eq "0"}
                      <div class="tabs">
                        ...
                        <div class="table-container yellow index orders"> 
                            <table width="100%"> 
                              <thead class="topics icons"> 
                                <tr> 
                                  <td class="first"><a href="{$baseurl}/manage_orders?s={$s}&b=id&a={if $a eq "asc"}desc{else}asc{/if}">{$lang140}</a></td> 
                                  <td class="amount"><a href="{$baseurl}/manage_orders?s{$s}=&b=status&a={if $a eq "asc"}desc{else}asc{/if}">{$lang191}</a></td> 
                                  <td class="last"><a href="{$baseurl}/manage_orders?s={$s}&b=date&a={if $a eq "asc"}desc{else}asc{/if}">{$lang360}</a></td> 
                                </tr> 
                              </thead> 
                              <tbody> 
